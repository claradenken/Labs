...
select account_id, trans_id, date, type, amount from bank.trans
...
# QUERY 17 (ANSWER DOESN'T MATCH, but answer seems to be right according to TA)
...
select date from bank.loan				# check if dates are there
	order by date desc;

#select date, duration, loan_id from bank.loan
#	where date = 971231
